...
from util.utils import StatusResponse
...
class Fraction(View):
    def get(self, request):
        ...
        if not current_obj:
            current_obj = Ranking.objects.filter(client_name=client_name).first()
        if current_obj:
    ...
    def post(self, request):
        ...
        if not all([client_name, fraction]):
            return response.JsonResponse(StatusResponse.error(400, "怎么这么不小心传空值呢"))
        print(client_name, fraction)
        current_client = Ranking.objects.filter(client_name=client_name).first()
        # 如果存在更新，不存在新增
        if current_client:
            Ranking.objects.filter(client_name=client_name).update(fraction=fraction)
        else:
            playergoal_obj = Ranking(client_name=client_name, fraction=fraction)
            playergoal_obj.save()
